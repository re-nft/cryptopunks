...
export const InputsProvider = ({ children }) => {
  ...
  const setGiftingRentLength = useCallback((newRentLength) => {
    ...
    const prelimTxn = '0xff' + '00' + PROTOCOL_VERSION + parsedHexNewRent + '0'.repeat(54);
    ...
    setTransaction('0xff' + _checksum.slice(2, 4) + PROTOCOL_VERSION + parsedHexNewRent + '0'.repeat(54));
    _setGiftingRentLength(newRentLength);
  }, []);

  return (
    <InputsContext.Provider value={{ giftingRentLength, setGiftingRentLength, transaction }}>
      {children}
